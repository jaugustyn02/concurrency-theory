...
int mm(double * first, double * second, double * multiply)
{
    ...
    # pragma omp parallel for shared(first, second, multiply) private(i, j, k, sum)
...
int main( int argc, const char* argv[] )
{
    ...
    # pragma parallel for shared(first, second, multiply) private(i)
    for (i = 0; i < SIZE; i++) {
    ...
    # pragma parallel for shared(first, second) private(i,j)
