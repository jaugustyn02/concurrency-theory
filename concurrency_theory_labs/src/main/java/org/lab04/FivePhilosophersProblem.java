...
//        Philosopher.enablePrinting = true;
        int[] N = new int[]{10, 25, 100};
        ...
        for (Philosopher.PhilosopherType type: Philosopher.PhilosopherType.values()) {
            ...
            for (int numOfPhilosophers : N) {
                ...
                try {
                    Thread.sleep(5000); // Threads running time - 5 sec
                ...
                timer.printTimes();
                timer.saveTimeMeasurementsAsCSV("src/main/results_lab04/"+type+"_"+numOfPhilosophers+".csv");
            }
        }
    }
}
